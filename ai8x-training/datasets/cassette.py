...
class cassette(Dataset):
    ...
    labels = ['black6', 'black8', 'black13', 'crystal', 'metal', 'station', 'white6', 'white8', 'whitebake']
    label_to_id_map = {k: v for v, k in enumerate(labels)}
    label_to_folder_map = {'black6': 'black6', 'black8': 'black8','black13' : 'black13', 'crystal' : 'crystal',
                           'metal' : 'metal', 'station' : 'station', 'white6' : 'white6', 'white8' : 'white8', 'whitebake' : 'whitebake'}

    def __init__(self, root_dir, d_type, transform=None,
                 resize_size=(128, 128), augment_data=False):
        self.root_dir = root_dir
        self.data_dir = os.path.join(root_dir, 'cassette', d_type) #name of dataset

        if not self.__check_cassette_data_exist():
        ...
        if d_type == 'train' and augment_data:
            self.album_transform = album.Compose([
                ...
                album.ShiftScaleRotate(shift_limit=0.05, scale_limit=0.05, rotate_limit=15, p=0.35), #defualt p=0.5
                album.RandomCrop(height=resize_size[0], width=resize_size[1]),
                album.HorizontalFlip(p=0.5),
                album.VerticalFlip(p=0.25), #add
    ...
    def __check_cassette_data_exist(self):
...
def get_cassette_dataset(data, load_train, load_test):
    ...
    if load_train:
        train_dataset = cassette(root_dir=data_dir, d_type='train',
                                   transform=transform, augment_data=True)
    else:
        train_dataset = None

    if load_test:
        test_dataset = cassette(root_dir=data_dir, d_type='test', transform=transform)
    else:
        test_dataset = None
...
datasets = [
    {
        'name': 'cassette',  #name of dataset
        'input': (3, 128, 128),
        'output': ('black6', 'black8', 'black13', 'crystal', 'metal', 'station', 'white6', 'white8', 'whitebake'),
        'loader': get_cassette_dataset,
    },
]
